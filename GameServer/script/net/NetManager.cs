...
namespace GameServer.script.net
{
    public class NetManager
    {
        ...
        {
            MethodInfo mei = typeof(GameServer.script.logic.EventHandler).GetMethod("OnTimer");
            object[] ob = { };
            mei.Invoke(null, ob);
        }
        ...
        public static void ReadClientfd(Socket clientfd)
        {
            ...
            int count = 0;
            //缓冲区不够，清除，若依旧不够，只能返回
            //缓冲区长度只有1024，单条协议超过缓冲区长度时会发送错误，根据需要调整长度
        ...
        public static void OnReceiveData(ClientState state)
        {
            ...
            Int16 bodyLength = (Int16)((bytes[readBuff.readIdx + 1] << 8) | bytes[readBuff.readIdx]); //!!!!这段不一样!!!!!
            //消息体
            ...
            Console.WriteLine("Recive" + protoName);
            if(mi!=null)
            {
                mi.Invoke(null, o);
            }
        ...
        {
            //事件分发
            MethodInfo mei = typeof(GameServer.script.logic.EventHandler).GetMethod("OnDisconnect");
            object[] ob = { state };
            mei.Invoke(null, ob);
            //关闭
            state.socket.Close();
